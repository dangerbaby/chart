...
if (mod(h.time_step-2,p.jumpplot)==0)
  if h.time_step==2
    ...
    title(strrep(p.plot.vidsavename,'_','-'),'interpreter','latex','fontsize',fs)
    ...
    if isfield(g,'struct')&g.struct.iover==1&mean(p.wave.Hrms)>0
      plot(g.struct.crest_x*[1 1],[g.struct.crest_elev interp1(g.x,g.zb,g.struct.crest_x)-.4],'-','color',[.8 .4 .3],'linewidth',2)
    end
    if iplotu
      inds = [1:1:length(g.x)];
