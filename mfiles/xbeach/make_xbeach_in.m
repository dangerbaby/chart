  ...
  for j = 1:length(bc) % loop over MC realizations
    ...
    for l = 1:length(reaches(i).height_dune)% loop over build configs
      for k = 1:length(bc(j).timeseries)% loop over storms
        disp(['Making XBEACH in structure, Reach: ',num2str(i),'  Montecar: ',num2str(j),'  Storm: ',num2str(k),'  Buildconfig: ',num2str(l)])
    ...
    save([g.name,'/work/infiles/',dirname,'/','xbin.mat'],'in');
  end
end
