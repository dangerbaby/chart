function make_infile(in,g)
% function makeinfile(in)


%First check for NaN
if max(isnan([in.x(:);in.zb(:);in.Tp(:);in.Hrms(:);in.swlbc(:);in.angle(:)]))
  error('The in structure contains NaN')
end
% save the in structure --just in case
%save(['./work/infiles/',in.dirname,'/',in.name,'.mat'],'in');

fid = fopen([g.name,'/work/infiles/',in.dirname,'/',in.name '.infile'],'w');
fprintf(fid,'%i \n',length(in.header));
